...
export const createPost = asyncHandler(async (req, res, next) => {
  const { title, content, author, category, createdBy, description } = req.body;
  ...
  if (mongoose.Types.ObjectId.isValid(category)) {
    // client sent categoryId directly
    categoryId = category;
  } else {
    // client sent category name (fast exact match, lowercase)
    const catName = category.trim().toLowerCase();
    let foundCategory = await Category.findOne({ slug: slugify(catName) });
  ...
  if (req.file) {
    try {
      ...
      await fs.rm(req.file.path); // remove temp file
    } catch (error) {
...
export const updatePost = asyncHandler(async (req, res, next) => {
    ...
    // If category is a name, find its ID
    if (categoryId && !mongoose.Types.ObjectId.isValid(categoryId)) {
      const foundCategory = await Category.findOne({ name: categoryId });
      if (!foundCategory) {
    ...
    const blogData = {
      ...
      category: categoryId, // ✅ this will be ObjectId string
    };
